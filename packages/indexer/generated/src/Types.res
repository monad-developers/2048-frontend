...
type loaderContext = {
  ...
  @as("GameMove") gameMove: entityLoaderContext<Entities.GameMove.t, Entities.GameMove.indexedFieldOperations>,
  @as("IndexerStatus") indexerStatus: entityLoaderContext<Entities.IndexerStatus.t, Entities.IndexerStatus.indexedFieldOperations>,
...
type handlerContext = {
  ...
  @as("GameMove") gameMove: entityHandlerContext<Entities.GameMove.t>,
  @as("IndexerStatus") indexerStatus: entityHandlerContext<Entities.IndexerStatus.t>,
...
@genType.as("GameMove")
type gameMove = Entities.GameMove.t
@genType.as("IndexerStatus")
type indexerStatus = Entities.IndexerStatus.t
@genType.as("Player")
...
module Transaction = {
  @genType
  type t = {hash: string, from: option<Address.t>, gasUsed: bigint, effectiveGasPrice: bigint, gasPrice: option<bigint>}

  let schema = S.object((s): t => {hash: s.field("hash", S.string), from: s.field("from", S.nullable(Address.schema)), gasUsed: s.field("gasUsed", BigInt.nativeSchema), effectiveGasPrice: s.field("effectiveGasPrice", BigInt.nativeSchema), gasPrice: s.field("gasPrice", S.nullable(BigInt.nativeSchema))})
}
...
module AggregatedTransaction = {
  @genType
  type t = {effectiveGasPrice: bigint, from: option<Address.t>, gasPrice: option<bigint>, gasUsed: bigint, hash: string}
}
...
module Monad2048 = {
let abi = Ethers.makeAbi((%raw(`[{"type":"event","name":"NewGame","inputs":[{"name":"player","type":"address","indexed":true},{"name":"id","type":"bytes32","indexed":true},{"name":"board","type":"uint256","indexed":false}],"anonymous":false},{"type":"event","name":"NewMove","inputs":[{"name":"player","type":"address","indexed":true},{"name":"id","type":"bytes32","indexed":true},{"name":"move","type":"uint256","indexed":false},{"name":"result","type":"uint256","indexed":false}],"anonymous":false}]`): Js.Json.t))
let eventSignatures = ["NewGame(address indexed player, bytes32 indexed id, uint256 board)", "NewMove(address indexed player, bytes32 indexed id, uint256 move, uint256 result)"]
@genType type chainId = [#143]
let contractName = "Monad2048"

module NewGame = {

let id = "0x3d6c02e74ba05913874ba8d477497510df840bf93bfa5a2a8d8943766678a0e3_3"
let sighash = "0x3d6c02e74ba05913874ba8d477497510df840bf93bfa5a2a8d8943766678a0e3"
let name = "NewGame"
let contractName = contractName

@genType
type eventArgs = {player: Address.t, id: string, board: bigint}
@genType
type block = Block.t
@genType
...
let paramsRawEventSchema = S.object((s): eventArgs => {player: s.field("player", Address.schema), id: s.field("id", S.string), board: s.field("board", BigInt.schema)})
...
@genType
type eventFilter = {@as("player") player?: SingleOrMultiple.t<Address.t>, @as("id") id?: SingleOrMultiple.t<string>}
...
let register = (): Internal.evmEventConfig => {
  let {getEventFiltersOrThrow, filterByAddresses} = LogSelection.parseEventFiltersOrThrow(~eventFilters=handlerRegister->EventRegister.getEventFilters, ~sighash, ~params=["player","id",], ~topic1=(_eventFilter) => _eventFilter->Utils.Dict.dangerouslyGetNonOption("player")->Belt.Option.mapWithDefault([], topicFilters => topicFilters->Obj.magic->SingleOrMultiple.normalizeOrThrow->Belt.Array.map(TopicFilter.fromAddress)), ~topic2=(_eventFilter) => _eventFilter->Utils.Dict.dangerouslyGetNonOption("id")->Belt.Option.mapWithDefault([], topicFilters => topicFilters->Obj.magic->SingleOrMultiple.normalizeOrThrow->Belt.Array.map(TopicFilter.castToHexUnsafe)))
  {
    ...
    convertHyperSyncEventArgs: (decodedEvent: HyperSyncClient.Decoder.decodedEvent) => {player: decodedEvent.indexed->Js.Array2.unsafe_get(0)->HyperSyncClient.Decoder.toUnderlying->Utils.magic, id: decodedEvent.indexed->Js.Array2.unsafe_get(1)->HyperSyncClient.Decoder.toUnderlying->Utils.magic, board: decodedEvent.body->Js.Array2.unsafe_get(0)->HyperSyncClient.Decoder.toUnderlying->Utils.magic, }->(Utils.magic: eventArgs => Internal.eventParams),
    id,
  name,
  contractName,
...
let sighash = "0x14fe4197a69dbcc73d79fcfd259bd1f097640dd2c2fe016ff232c416e17ce842"
let name = "NewMove"
let contractName = contractName

@genType
type eventArgs = {player: Address.t, id: string, move: bigint, result: bigint}
@genType
type block = Block.t
@genType
type transaction = Transaction.t
...
let paramsRawEventSchema = S.object((s): eventArgs => {player: s.field("player", Address.schema), id: s.field("id", S.string), move: s.field("move", BigInt.schema), result: s.field("result", BigInt.schema)})
let blockSchema = Block.schema
let transactionSchema = Transaction.schema
...
let register = (): Internal.evmEventConfig => {
  let {getEventFiltersOrThrow, filterByAddresses} = LogSelection.parseEventFiltersOrThrow(~eventFilters=handlerRegister->EventRegister.getEventFilters, ~sighash, ~params=["player","id",], ~topic1=(_eventFilter) => _eventFilter->Utils.Dict.dangerouslyGetNonOption("player")->Belt.Option.mapWithDefault([], topicFilters => topicFilters->Obj.magic->SingleOrMultiple.normalizeOrThrow->Belt.Array.map(TopicFilter.fromAddress)), ~topic2=(_eventFilter) => _eventFilter->Utils.Dict.dangerouslyGetNonOption("id")->Belt.Option.mapWithDefault([], topicFilters => topicFilters->Obj.magic->SingleOrMultiple.normalizeOrThrow->Belt.Array.map(TopicFilter.castToHexUnsafe)))
  {
    ...
    blockSchema: blockSchema->(Utils.magic: S.t<block> => S.t<Internal.eventBlock>),
    transactionSchema: transactionSchema->(Utils.magic: S.t<transaction> => S.t<Internal.eventTransaction>),
    convertHyperSyncEventArgs: (decodedEvent: HyperSyncClient.Decoder.decodedEvent) => {player: decodedEvent.indexed->Js.Array2.unsafe_get(0)->HyperSyncClient.Decoder.toUnderlying->Utils.magic, id: decodedEvent.indexed->Js.Array2.unsafe_get(1)->HyperSyncClient.Decoder.toUnderlying->Utils.magic, move: decodedEvent.body->Js.Array2.unsafe_get(0)->HyperSyncClient.Decoder.toUnderlying->Utils.magic, result: decodedEvent.body->Js.Array2.unsafe_get(1)->HyperSyncClient.Decoder.toUnderlying->Utils.magic, }->(Utils.magic: eventArgs => Internal.eventParams),
    id,
  name,
  contractName,
  isWildcard: (handlerRegister->EventRegister.isWildcard),
